...
def lambda_handler(event, context):
    try:
        for record in event["Records"]:
            ...
            input_text = (
                f"You are a secure coding assistant. Respond only with valid JSON. "
                f"Generate a multiple choice quiz for the OWASP topic: {category}. "
                f"Do not include any explanations or text outside the JSON.\n\n"
                f"The JSON format must be:\n"
                f"{{\n"
                f'  "questions": [\n'
                f"    {{\n"
                f'      "question": "string",\n'
            ...
            table.update_item(
                ...
                UpdateExpression="SET #s = :s, #q = :q, #r = :r, updatedAt = :t",
